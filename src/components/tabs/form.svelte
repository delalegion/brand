<script lang="ts">
   ...
   export let form;
      ...
      <div class="flex py-2 md:py-5 lg:py-10">
         <h5 class="font-medium text-h6 md:text-h6 lg:text-h6 xl:text-h5 text-dark-200">Pytania? Napisz do mnie a postaram się<br /> odpowiedzieć jak najszybciej to możliwe!</h5>
      </div>

      <form method="POST" action="?/form" class="flex py-10 max-w-[580px]" use:enhance>
         <div class="col-span-3">

            {#if form?.formSuccess}<p class="w-full p-4 bg-green-800 rounded-lg text-gray-200 col-span-6 lg:col-span-12 mb-4" transition:fade={{ easing: sineIn, duration: 300 }}>Wiadomość wysłana. Odpowiem jak najszybciej będzie to mozliwe. W razie potrzeby szybkiego kontaktu zapraszam dzwonić na podany numer telefonu: 782788066</p>{/if}
            {#if form?.formNameError}<p class="w-full p-4 bg-red-800 rounded-lg text-gray-200 col-span-6 lg:col-span-12 mb-4" transition:fade={{ easing: sineIn, duration: 300 }}>Pole imię / firma jest wymagane i musi zawierać conajmniej 5 znaków.</p>{/if}
            {#if form?.formEmailError}<p class="w-full p-4 bg-red-800 rounded-lg text-gray-200 col-span-6 lg:col-span-12 mb-4" transition:fade={{ easing: sineIn, duration: 300 }}>Pole email jest wymagane i musi zawierać prawidłowy adres email.</p>{/if}
            {#if form?.formPhoneError}<p class="w-full p-4 bg-red-800 rounded-lg text-gray-200 col-span-6 lg:col-span-12 mb-4" transition:fade={{ easing: sineIn, duration: 300 }}>Pole numer telefonu może zawierać jedynie cyfry.</p>{/if}
            {#if form?.formMessageError}<p class="w-full p-4 bg-red-800 rounded-lg text-gray-200 col-span-6 lg:col-span-12 mb-4" transition:fade={{ easing: sineIn, duration: 300 }}>Pole treść wiadomości jest wymagane i musi zawierać conajmniej 5 znaków.</p>{/if}
            {#if form?.formCheckboxError}<p class="w-full p-4 bg-red-800 rounded-lg text-gray-200 col-span-6 lg:col-span-12 mb-4" transition:fade={{ easing: sineIn, duration: 300 }}>Aby wysłać wiadomość musisz zaakceptować zgodę na przetwarzanie danych.</p>{/if}

            <div class="flex py-2 gap-4 flex-col">
               <div class="relative h-16 w-full min-w-[200px] inp">
						<input placeholder="Email *" name="email" type="email"
						  class="peer h-full w-full border-2 placeholder:text-dark-400 border-dark-700 rounded-md bg-transparent px-3 md:px-6 pt-5 pb-0 font-primary font-semibold text-p1 text-dark-200 outline outline-0 transition-all placeholder-shown:border-dark-150 focus:border-dark-500 focus:outline-0 disabled:border-0 disabled:bg-dark-50 placeholder:opacity-0 focus:placeholder:opacity-100" />
						<label
